...
# ===========================================================================
# =                                                                         =
# = File added to Paltax by Koen Waggeveld                                  =
...
from paltax.vit_jax import models_resnet           # KCW
...
class VisionTransformer(nn.Module):
  ...
  transformer: Any
  hidden_size: int
  dtype: Any = jnp.float32      # KCW
  resnet: Optional[Any] = None
#   representation_size: Optional[int] = None
  num_outputs: Optional[int] = None     # KCW
...
#   def __call__(self, inputs, *, train):
  def __call__(self, inputs, train: bool = True):        # KCW
    x = inputs
                          # KCW: to ensure 'batch_stats' are created (expected by paltax)
    ...
    if self.num_outputs is not None:
      x = nn.Dense(features=self.num_outputs, name='pre_logits')(x)
      # x = nn.tanh(x)    # KCW: Seems like a bad idea to get all outputs in (0,1)
    else:
